...
Fastify app ([src/server.ts](apps/api/src/server.ts)) registering one route module per domain under `src/routes/` (auth, projects, sessions, ideas, publications, search, messages, notifications, follows, report, trending, settings, ws, pulls, session-proxy, session-subdomain). In production the built frontend is served from the same Fastify instance (`apps/web/dist`), with `/api/*` stripped to route prefixes via `rewriteUrl`; in dev, Vite proxies `/api` and `/ws` to the API (see [apps/web/vite.config.ts](apps/web/vite.config.ts)).
